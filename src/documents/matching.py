...
def match_equipment(document: Document, classifier: DocumentClassifier, user=None):
    pred_id = classifier.predict_equipment(document.content) if classifier else None

    if user is None and document.owner is not None:
        user = document.owner

    if user is not None:
        equipments = get_objects_for_user_owner_aware(
            user,
            "documents.view_equipment",
            Equipment,
        )
    else:
...
def existing_document_matches_workflow(
    ...
    if (
        ...
        # Document equipment vs trigger has_equipment
    if (
        trigger.filter_has_equipment is not None
        and document.equipment != trigger.filter_has_equipment
    ):
        reason = (
            f"Document equipment {document.equipment} does not match {trigger.filter_has_equipment}",
        )
        trigger_matched = False

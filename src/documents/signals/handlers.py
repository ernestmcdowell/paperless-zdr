...
def set_equipment(
    ...
    **kwargs,
    ...
    if document.equipment and not replace:
        return

    potential_equipments = matching.match_equipment(document, classifier)

    potential_count = len(potential_equipments)
    selected = potential_equipments[0] if potential_equipments else None
    if potential_count > 1:
        if use_first:
            logger.debug(
                f"Detected {potential_count} potential equipment, "
                f"so we've opted for {selected}",
                extra={"group": logging_group},
            )
        else:
    ...
    if selected or replace:
        if suggest:
            ...
            print(f"Suggest equipment {selected}")
        else:
            logger.info(
                f"Assigning equipment {selected} to {document}",
                extra={"group": logging_group},
            )

            document.equipment = selected
            document.save(update_fields=("equipment",))

...
from documents.models import Equipment
...
class EquipmentSerializer(MatchingModelSerializer, OwnedObjectSerializer):
    last_equipment = serializers.DateTimeField(read_only=True)
    vin_number = serializers.CharField(read_only=False)
    class Meta:
        model = Equipment
        fields = (
            "id",
...
class EquipmentField(serializers.PrimaryKeyRelatedField):
    def get_queryset(self):
...
class DocumentSerializer(
    ...
    equipment = EquipmentField(allow_null=True)
...
class BulkEditSerializer(DocumentListSerializer, SetPermissionsMixin):
    ...
    def validate_method(self, method):
        ...
        if method == "set_equipment":
            return bulk_edit.set_equipment
    ...
    def _validate_parameters_equipment(self, parameters):
        if "equipment" in parameters:
            equipment_id = parameters["equipment"]
            ...
            raise serializers.ValidationError("equipment not specified")
    ...
    def validate(self, attrs):
        ...
        if method == bulk_edit.set_equipment:
            self._validate_parameters_equipment(parameters)
...
class WorkflowActionSerializer(serializers.ModelSerializer):
    ...
    assign_equipment = EquipmentField(allow_null=True, required=False)

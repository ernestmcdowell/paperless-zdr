...
def set_equipment(doc_ids, equipment):
    if equipment:
        equipment = Equipment.objects.get(id=equipment)

    qs = Document.objects.filter(Q(id__in=doc_ids) & ~Q(equipment=equipment))
    affected_docs = [doc.id for doc in qs]
    qs.update(equipment=equipment)
...
def add_tag(doc_ids, tag):
    ...
    DocumentTagRelationship.objects.bulk_create(
        [DocumentTagRelationship(document_id=doc, tag_id=tag) for doc in
         affected_docs],
    )

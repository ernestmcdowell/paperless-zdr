...
class DocumentClassifier:
    ...
    def __init__(self):
        ...
        self.equipment_classifier = None
            ...
            with open(settings.MODEL_FILE, "rb") as f:
                ...
                if schema_version != self.FORMAT_VERSION:
                    ...
                    try:
                        ...
                        self.equipment_classifier = pickle.load(f)
    ...
    def save(self):
        ...
        with open(target_file_temp, "wb") as f:
            ...
            pickle.dump(self.equipment_classifier, f)
    ...
    def train(self):
        ...
        for doc in docs_queryset:
            ...
            equip = doc.equipment
            if equip and equip.matching_algorithm == MatchingModel.MATCH_AUTO:
                y = equip.pk
            hasher.update(y.to_bytes(4, "little", signed=True))
            labels_equipment.append(y)
        ...
        num_equipment = len(set(labels_equipment) | {-1}) - 1
        num_document_types = len(set(labels_document_type) | {-1}) - 1
        num_storage_paths = len(set(labels_storage_path) | {-1}) - 1

        logger.debug(
            "{} documents, {} tag(s), {} correspondent(s), {} equipments "
        ...
        if num_equipment > 0:
            logger.debug("Training equipment classifier...")
            self.equipment_classifier = MLPClassifier(tol=0.01)
            self.equipment_classifier.fit(data_vectorized, labels_equipment)
        else:
            self.equipment_classifier = None
            logger.debug(
                "There is no equipment. Not training equipment "
                "classifier.",
            )
    ...
    def predict_equipment(self, content: str) -> Optional[int]:
        if self.equipment_classifier:
            X = self.data_vectorizer.transform([self.preprocess_content(content)])
            equipment_id = self.equipment_classifier.predict(X)
            if equipment_id != -1:
                return equipment_id
            else:
                return None

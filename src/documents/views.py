...
from documents.matching import match_equipment
...
from documents.filters import EquipmentFilterSet
from documents.serialisers import EquipmentSerializer
...
class EquipmentViewSet(ModelViewSet, PassUserMixin):
    model = Equipment
    queryset = (
        Equipment.objects.annotate(
            document_count=Count("documents"),
            last_equipment=Max("documents__created"),
        )
        .select_related("owner")
        .order_by(Lower("name"))
    )
...
class DocumentViewSet(
    ...
    search_fields = ("title", "correspondent__name", "equipment__name", "content")
    ordering_fields = (
        "id",
        "title",
        "correspondent__name",
    ...
    def get_queryset(self):
        return (
            ...
            .select_related("correspondent", "storage_path", "document_type", "equipment", "owner")
        ...
        return Response(
            {
                ...
                "equipments": [
                    c.id for c in
                    match_equipment(doc, classifier, request.user)
                ],

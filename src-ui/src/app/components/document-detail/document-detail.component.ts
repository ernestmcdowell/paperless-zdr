...
import {
  ...
  FILTER_HAS_TAGS_ALL,
  FILTER_STORAGE_PATH, FILTER_EQUIPMENT_VIN,
  ...
  implements OnInit, OnDestroy, DirtyComponent
{
  ...
  createCorrespondent(newName: string) {
    ...
    createEquipment(newName: string) {
    var modal = this.modalService.open(EquipmentEditDialogComponent, {
      backdrop: 'static',
    })
    modal.componentInstance.dialogMode = EditDialogMode.CREATE
    if (newName) modal.componentInstance.object = { name: newName }
    modal.componentInstance.succeeded
      .pipe(
        switchMap((newEquipment) => {
          return this.equipmentService
            .listAll()
            .pipe(
              map((equipments) => ({ newEquipment, equipments }))
            )
        })
      )
      .pipe(takeUntil(this.unsubscribeNotifier))
      .subscribe(({ newEquipment, equipments }) => {
        this.equipments = equipments.results
        this.documentForm.get('equipment').setValue(newEquipment.id)
      })
  }
  ...
  filterDocuments(items: ObjectWithId[] | NgbDateStruct[]) {
    const filterRules: FilterRule[] = items.flatMap((i) => {
      ...
      } else if (i.hasOwnProperty('last_equipment')) {
        // Equipment
        return {
          rule_type: FILTER_EQUIPMENT,
          value: (i as Equipment).name.toString(),
        }
      } else if (i.hasOwnProperty('vin_number')) {
        // Equipment
        return {
          rule_type: FILTER_EQUIPMENT_VIN,
          value: (i as Equipment).vin_number.toString(),
        }
  ...
  this.document.custom_fields?.forEach((fieldInstance) => {
    const field = this.customFields?.find(f => f.id === fieldInstance.field);
    if (field) {
      fieldInstance.name = field.name; // Set the name
    }
    this.customFieldFormFields.push(
      new FormGroup({
        field: new FormControl(field?.id),
        value: new FormControl(fieldInstance.value),
        name: new FormControl(fieldInstance.name)
      }),
      { emitEvent }
    );
  });

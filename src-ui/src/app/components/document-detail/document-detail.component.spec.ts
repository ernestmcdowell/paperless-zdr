...
import {
  ...
  FILTER_CREATED_BEFORE, FILTER_EQUIPMENT,
...
describe('DocumentDetailComponent', () => {
  ...
  it('should support creating equipment', () => {
    initNormally()
    let openModal: NgbModalRef
    modalService.activeInstances.subscribe((modal) => (openModal = modal[0]))
    const modalSpy = jest.spyOn(modalService, 'open')
    component.createEquipment('NewEquipment13')
    expect(modalSpy).toHaveBeenCalled()
    openModal.componentInstance.succeeded.next({
      id: 12,
      name: 'NewEquipment13',
    })
    expect(component.documentForm.get('equipment').value).toEqual(13)
  })
  ...
  it('should support quick filtering by equipment', () => {
    initNormally()
    const object = {
      id: 23,
      name: 'Equipment23',
      last_equipment: new Date().toISOString(),
      vin_number: 'some23423vinS',
    } as Equipment
    const qfSpy = jest.spyOn(documentListViewService, 'quickFilter')
    component.filterDocuments([object])
    expect(qfSpy).toHaveBeenCalledWith([

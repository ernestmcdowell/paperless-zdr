  ...
  implements OnInit, OnDestroy
{
  ...
  equipmentSelectionModel = new FilterableDropdownSelectionModel()
  documentTypeSelectionModel = new FilterableDropdownSelectionModel()
  storagePathsSelectionModel = new FilterableDropdownSelectionModel()
  tagDocumentCounts: SelectionDataItem[]
  correspondentDocumentCounts: SelectionDataItem[]
  equipmentDocumentCounts: SelectionDataItem[]
  ...
  ngOnInit() {
    ...
    this.equipmentService
      .listAll()
      .pipe(first())
      .subscribe((result) => (this.equipments = result.results))
  ...
  openEquipmentDropdown() {
    this.documentService
      ...
      .pipe(first())
      .subscribe((s) => {
        this.equipmentDocumentCounts = s.selected_equipments
        this.applySelectionData(
          s.selected_equipments,
          this.equipmentSelectionModel
        )
      })
  }
  ...
  setEquipments(changedEquipments: ChangedItems) {
    if (
      changedEquipments.itemsToAdd.length == 0 &&
      changedEquipments.itemsToRemove.length == 0
    )
      return

    let equipment =
      changedEquipments.itemsToAdd.length > 0
        ? changedEquipments.itemsToAdd[0]
        : null
    ...
    if (this.showConfirmationDialogs) {
      ...
      modal.componentInstance.title = $localize`Confirm equipment assignment`
      if (equipment) {
        modal.componentInstance.message = $localize`This operation will assign the equipments "${equipment.name}" to ${this.list.selected.size} selected document(s).`
      } else {
        modal.componentInstance.message = $localize`This operation will remove the equipment from ${this.list.selected.size} selected document(s).`
      }
      modal.componentInstance.btnClass = 'btn-warning'
      modal.componentInstance.btnCaption = $localize`Confirm`
      modal.componentInstance.confirmClicked
        .pipe(takeUntil(this.unsubscribeNotifier))
        .subscribe(() => {
          this.executeBulkOperation(modal, 'set_equipment', {
            equipment: equipment ? equipment.id : null,
          })
        })
    } else {
      this.executeBulkOperation(null, 'set_equipment', {
        equipment: equipment ? equipment.id : null,
      })
    }
  }

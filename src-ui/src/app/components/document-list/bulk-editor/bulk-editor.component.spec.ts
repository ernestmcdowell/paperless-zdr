...
const selectionData: SelectionData = {
  ...
  selected_equipments: [{ id: 33, document_count: 1 }],
...
describe('BulkEditorComponent', () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            listAll: () =>
              of({
                results: [{ id: 33, name: 'equipment33' }],
              }),
          },
        },
        {
  ...
  it('should apply selection data to equipments menu', () => {
    jest.spyOn(permissionsService, 'currentUserCan').mockReturnValue(true)
    fixture.detectChanges()
    expect(
    ...
    jest
      .spyOn(documentService, 'getSelectionData')
      .mockReturnValue(of(selectionData))
    component.openEquipmentDropdown()
    expect(component.equipmentSelectionModel.items).toHaveLength(2)
    expect(component.equipmentSelectionModel.selectionSize()).toEqual(0)
  })
  ...
  it('should execute modify equipment bulk operation', () => {
    jest.spyOn(permissionsService, 'currentUserCan').mockReturnValue(true)
    jest
      .spyOn(documentListViewService, 'documents', 'get')
      .mockReturnValue([{ id: 3 }, { id: 4 }])
  ...
  it('should execute modify equipment bulk operation with confirmation dialog if enabled', () => {
    let modal: NgbModalRef
    modalService.activeInstances.subscribe((m) => (modal = m[0]))
    jest.spyOn(permissionsService, 'currentUserCan').mockReturnValue(true)
    jest
  ...
  it('should set modal dialog text accordingly for equipment edit confirmation', () => {
    let modal: NgbModalRef
    modalService.activeInstances.subscribe((m) => (modal = m[m.length - 1]))
    jest.spyOn(permissionsService, 'currentUserCan').mockReturnValue(true)
    jest
    ...
    component.setEquipments({
      itemsToAdd: [],
      itemsToRemove: [{ id: 101, name: 'Equipment 101' }],
    })
    expect(modal.componentInstance.message).toEqual(
      'This operation will remove the equipment from 2 selected document(s).'
    )
    modal.close()
    component.setEquipments({
      itemsToAdd: [{ id: 101, name: 'Equipment 101' }],
      itemsToRemove: [],
    })
    expect(modal.componentInstance.message).toEqual(
      'This operation will assign the equipment "Equipment 101" to 2 selected document(s).'
    )
  })

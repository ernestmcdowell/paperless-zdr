...
import {
  ...
  FILTER_DOES_NOT_HAVE_EQUIPMENT,
...
export class FilterEditorComponent implements OnInit, OnDestroy {
  ...
  set filterRules(value: FilterRule[]) {
    ...
    this.equipmentSelectionModel.clear(false)
    ...
    value.forEach((rule) => {
      switch (rule.rule_type) {
        ...
        case FILTER_HAS_EQUIPMENT_ANY:
          this.equipmentSelectionModel.logicalOperator = LogicalOperator.Or
          this.equipmentSelectionModel.intersection = Intersection.Include
          this.equipmentSelectionModel.set(
            rule.value ? +rule.value : null,
            ToggleableItemState.Selected,
            false
          )
          break
        case FILTER_DOES_NOT_HAVE_EQUIPMENT:
          this.equipmentSelectionModel.intersection = Intersection.Exclude
          this.equipmentSelectionModel.set(
            rule.value ? +rule.value : null,
            ToggleableItemState.Excluded,
            false
  ...
  get filterRules(): FilterRule[] {
    ...
    if (this.equipmentSelectionModel.isNoneSelected()) {
      filterRules.push({ rule_type: FILTER_EQUIPMENT, value: null })
    } else {
  ...
  set selectionData(selectionData: SelectionData) {
    ...
    this.equipmentDocumentCounts =
      selectionData?.selected_equipments ?? null
  ...
  toggleEquipment(equipmentId: number) {
    this.equipmentSelectionModel.toggle(equipmentId)
  }
  ...
  onEquipmentDropdownOpen() {
    this.equipmentSelectionModel.apply()
  }

...
import {
  ...
  FILTER_SHARED_BY_USER, FILTER_DOES_NOT_HAVE_EQUIPMENT, FILTER_HAS_EQUIPMENT_ANY, FILTER_EQUIPMENT,
...
import { EquipmentService } from '../../../services/rest/equipment.service'
...
describe('FilterEditorComponent', () => {
  ...
  it('should ingest filter rules for has equipment', fakeAsync(() => {
    expect(
      component.equipmentSelectionModel.getSelectedItems()
    ).toHaveLength(0)
    component.filterRules = [
      {
        rule_type: FILTER_EQUIPMENT,
        value: '12',
      },
    ]
    expect(component.equipmentSelectionModel.logicalOperator).toEqual(
      LogicalOperator.Or
    )
    expect(component.equipmentSelectionModel.intersection).toEqual(
      Intersection.Include
    )
    expect(component.equipmentSelectionModel.getSelectedItems()).toEqual([
      correspondents[0],
    ])
    component.toggleCorrespondent(12) // coverage
  }))
  ...
  it('should ingest filter rules for has any of correspondents', fakeAsync(() => {
    ...
    component.filterRules = [
      {
        rule_type: FILTER_HAS_EQUIPMENT_ANY,
        value: '12',
      },
      {
        rule_type: FILTER_HAS_EQUIPMENT_ANY,
  ...
  it('should ingest filter rules for does not have any of correspondents', fakeAsync(() => {
    expect(
      component.equipmentSelectionModel.getExcludedItems()
    ).toHaveLength(0)
    component.filterRules = [
      {
        rule_type: FILTER_DOES_NOT_HAVE_EQUIPMENT,
        value: '12',
      },
      {
        rule_type: FILTER_DOES_NOT_HAVE_EQUIPMENT,
    ...
    expect(component.equipmentSelectionModel.getExcludedItems()).toEqual(
      equipments
    )
    // coverage
    component.filterRules = [
  ...
  it('should convert user input to correct filter rules on correspondent selections', fakeAsync(() => {
    const equipmentFilterableDropdown = fixture.debugElement.queryAll(
      By.directive(FilterableDropdownComponent)
    )[1] // Corresp dropdown
    equipmentFilterableDropdown.triggerEventHandler('opened')
    const equipmentButtons = equipmentFilterableDropdown.queryAll(
      By.directive(ToggleableDropdownButtonComponent)
    )
    equipmentButtons[1].triggerEventHandler('toggle')
    equipmentButtons[2].triggerEventHandler('toggle')
    fixture.detectChanges()
    expect(component.filterRules).toEqual([
      {
        rule_type: FILTER_HAS_EQUIPMENT_ANY,
        value: equipments[0].id.toString(),
      },
      {
        rule_type: FILTER_HAS_EQUIPMENT_ANY,
        value: equipments[1].id.toString(),
      },
    ])
    const toggleIntersectionButtons = equipmentFilterableDropdown.queryAll(
      By.css('input[type=radio]')
    )
    toggleIntersectionButtons[1].nativeElement.checked = true
    toggleIntersectionButtons[1].triggerEventHandler('change')
  ...
  it('should convert user input to correct filter rules on correspondent select not assigned', fakeAsync(() => {
    const equipmentsFilterableDropdown = fixture.debugElement.queryAll(
      By.directive(FilterableDropdownComponent)
    )[1]
    equipmentsFilterableDropdown.triggerEventHandler('opened')
    const notAssignedButton = equipmentsFilterableDropdown.queryAll(
      By.directive(ToggleableDropdownButtonComponent)
    )[0]
  ...
  it('should generate filter names', () => {
    ...
    expect(component.generateFilterName()).toEqual('Without equipments')
